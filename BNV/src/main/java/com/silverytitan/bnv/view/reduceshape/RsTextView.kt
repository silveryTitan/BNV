package com.silverytitan.bnv.view.reduceshape
...
import com.silverytitan.bnv.R

class RsTextView : AppCompatTextView, View.OnTouchListener {
    @Type
    ...
    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : super(context, attrs,
            defStyleAttr) {
        context.obtainStyledAttributes(attrs, R.styleable.RsTextView, defStyleAttr, 0)
                .apply {
                    type = getInt(R.styleable.RsTextView_rs_type, TYPE_MASK)
                    shape = getInt(R.styleable.RsTextView_rs_shape, GradientDrawable.RECTANGLE)
                    innerRadius = getDimensionPixelSize(R.styleable.RsTextView_rs_innerRadius, 0)
                    innerRadiusRatio = getFloat(R.styleable.RsTextView_rs_innerRadiusRatio, 0f)
                    thickness = getDimensionPixelSize(R.styleable.RsTextView_rs_thickness, 0)
                    thicknessRatio = getFloat(R.styleable.RsTextView_rs_thicknessRatio, 0f)
                    normalBackgroundColor = getColor(
                            R.styleable.RsTextView_rs_normalBackgroundColor, 0)
                    pressedBackgroundColor = getColor(
                            R.styleable.RsTextView_rs_pressedBackgroundColor, 0)
                    disabledBackgroundColor = getColor(
                            R.styleable.RsTextView_rs_disabledBackgroundColor,
                            DEFAULT_DISABLE_BACKGROUND_COLOR)
                    selectedBackgroundColor = getColor(
                            R.styleable.RsTextView_rs_selectedBackgroundColor, 0)
                    strokeWidth = getDimensionPixelSize(R.styleable.RsTextView_rs_strokeWidth, 0)
                    dashWidth = getDimension(R.styleable.RsTextView_rs_dashWidth, 0f)
                    dashGap = getDimension(R.styleable.RsTextView_rs_dashGap, 0f)
                    normalStrokeColor = getColor(R.styleable.RsTextView_rs_normalStrokeColor, 0)
                    pressedStrokeColor = getColor(R.styleable.RsTextView_rs_pressedStrokeColor,
                            normalStrokeColor)
                    disabledStrokeColor = getColor(R.styleable.RsTextView_rs_disabledStrokeColor,
                            normalStrokeColor)
                    selectedStrokeColor = getColor(R.styleable.RsTextView_rs_selectedStrokeColor,
                            normalStrokeColor)
                    normalTextColor = getColor(R.styleable.RsTextView_rs_normalTextColor,
                            currentTextColor)
                    pressedTextColor = getColor(R.styleable.RsTextView_rs_pressedTextColor,
                            currentTextColor)
                    disabledTextColor = getColor(R.styleable.RsTextView_rs_disabledTextColor,
                            currentTextColor)
                    selectedTextColor = getColor(R.styleable.RsTextView_rs_selectedTextColor,
                            currentTextColor)
                    cornersRadius = getDimension(R.styleable.RsTextView_rs_cornersRadius, 0f)
                    cornersTopLeftRadius = getDimension(
                            R.styleable.RsTextView_rs_cornersTopLeftRadius, 0f)
                    cornersTopRightRadius = getDimension(
                            R.styleable.RsTextView_rs_cornersTopRightRadius, 0f)
                    cornersBottomLeftRadius = getDimension(
                            R.styleable.RsTextView_rs_cornersBottomLeftRadius, 0f)
                    cornersBottomRightRadius = getDimension(
                            R.styleable.RsTextView_rs_cornersBottomRightRadius, 0f)
                    val normalGradientColorsResourceId = getResourceId(
                            R.styleable.RsTextView_rs_normalGradientColors, 0)
                    if (normalGradientColorsResourceId != 0) {
                        normalGradientColors = resources.getIntArray(normalGradientColorsResourceId)
                    }
                    val pressedGradientColorsResourceId = getResourceId(
                            R.styleable.RsTextView_rs_pressedGradientColors, 0)
                    if (pressedGradientColorsResourceId != 0) {
                        pressedGradientColors = resources.getIntArray(
                                pressedGradientColorsResourceId)
                    }
                    val disabledGradientColorsResourceId = getResourceId(
                            R.styleable.RsTextView_rs_disabledGradientColors, 0)
                    if (disabledGradientColorsResourceId != 0) {
                        disabledGradientColors = resources.getIntArray(
                                disabledGradientColorsResourceId)
                    }
                    val selectedGradientColorsResourceId = getResourceId(
                            R.styleable.RsTextView_rs_selectedGradientColors, 0)
                    if (selectedGradientColorsResourceId != 0) {
                        selectedGradientColors = resources.getIntArray(
                                selectedGradientColorsResourceId)
                    }
                    gradientOrientation = getInt(R.styleable.RsTextView_rs_gradientOrientation,
                            GRADIENT_ORIENTATION_TOP_BOTTOM)
                    gradientType = getInt(R.styleable.RsTextView_rs_gradientType,
                            GradientDrawable.LINEAR_GRADIENT)
                    gradientCenterX = getDimension(R.styleable.RsTextView_rs_gradientCenterX, 0f)
                    gradientCenterY = getDimension(R.styleable.RsTextView_rs_gradientCenterY, 0f)
                    gradientRadius = getDimension(R.styleable.RsTextView_rs_gradientRadius, 0f)
                    maskBackgroundColor = getColor(R.styleable.RsTextView_rs_maskBackgroundColor,
                            DEFAULT_MASK_BACKGROUND_COLOR)
                    cancelOffset = getDimensionPixelSize(R.styleable.RsTextView_rs_cancelOffset,
...
//    }
    ...
    private fun init() {
        val normalDrawable = getDrawable(normalBackgroundColor, normalStrokeColor,
        ...
        when (type) {
            TYPE_MASK -> {
                pressedDrawable = getDrawable(normalBackgroundColor, normalStrokeColor,
                        normalGradientColors).apply {
                    colorFilter = PorterDuffColorFilter(maskBackgroundColor,
                            PorterDuff.Mode.SRC_ATOP)
                }
                disabledDrawable = getDrawable(disabledBackgroundColor, disabledBackgroundColor)
            }
            TYPE_SELECTOR -> {
                pressedDrawable = getDrawable(pressedBackgroundColor, pressedStrokeColor,
                        pressedGradientColors)
                disabledDrawable = getDrawable(disabledBackgroundColor, disabledStrokeColor,
                        disabledGradientColors)
            }
        }
        selectedDrawable = getDrawable(selectedBackgroundColor, selectedStrokeColor,
    ...
    private fun getDrawable(
        backgroundColor: Int, strokeColor: Int, gradientColors: IntArray? = null): GradientDrawable {
        // 背景色相关
        ...
        // 描边相关
        if (strokeColor != 0) {
            (drawable.mutate() as GradientDrawable).setStroke(strokeWidth, strokeColor, dashWidth,
                    dashGap)
        }

        // 圆角相关
        setupCornersRadius(drawable, cornersRadius, cornersTopLeftRadius, cornersTopRightRadius,
                cornersBottomRightRadius, cornersBottomLeftRadius)

        // 渐变相关
        (drawable.mutate() as GradientDrawable).gradientType = gradientType
        if (gradientCenterX != 0.0f || gradientCenterY != 0.0f) {
            (drawable.mutate() as GradientDrawable).setGradientCenter(gradientCenterX,
                    gradientCenterY)
        }
    ...
    fun setTextColor(
        normalTextColor: Int = this.normalTextColor, pressedTextColor: Int = this.pressedTextColor, disabledTextColor: Int = this.disabledTextColor) {
    ...
    fun setCornersRadius(cornersRadius: Float = this.cornersRadius) {
        setCornersRadius(cornersRadius, 0f, 0f, 0f, 0f)
    }
    ...
    fun setStroke(
        strokeWidth: Int = this.strokeWidth, dashWidth: Float = this.dashWidth, dashGap: Float = this.dashGap, normalStrokeColor: Int = this.normalStrokeColor, pressedStrokeColor: Int = this.pressedStrokeColor, disabledStrokeColor: Int = this.disabledStrokeColor, selectedStrokeColor: Int = this.selectedStrokeColor) {
    ...
    private fun setupCornersRadius(
        drawable: GradientDrawable, cornersRadius: Float, cornersTopLeftRadius: Float, cornersTopRightRadius: Float, cornersBottomRightRadius: Float, cornersBottomLeftRadius: Float) {
        if (cornersRadius > 0f) {
    ...
    fun setColor(backgroundColor: Int) {
        if (background !is StateListDrawable) return
        if (background.constantState !is DrawableContainer.DrawableContainerState) return
        val dcs: DrawableContainer.DrawableContainerState = background.constantState as DrawableContainer.DrawableContainerState
        val children = dcs.children
        if (children.isNullOrEmpty()) return
