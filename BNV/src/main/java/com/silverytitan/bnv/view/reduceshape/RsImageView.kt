...
class RsImageView : ShapeableImageView, View.OnClickListener, View.OnTouchListener {
    companion object {
    ...
    @Target(AnnotationTarget.FIELD, AnnotationTarget.FUNCTION, AnnotationTarget.PROPERTY,
            AnnotationTarget.VALUE_PARAMETER) @Retention(AnnotationRetention.SOURCE)
    @IntDef(STYLE_NORMAL, STYLE_ROUNDED, STYLE_OVAL) annotation class Style

    @Target(AnnotationTarget.FIELD, AnnotationTarget.FUNCTION, AnnotationTarget.PROPERTY,
            AnnotationTarget.VALUE_PARAMETER) @Retention(AnnotationRetention.SOURCE)
    @IntDef(TYPE_MASK, TYPE_ALPHA, TYPE_SELECTOR, TYPE_CHECKBOX) annotation class Type
    ...
    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : super(context, attrs,
            defStyleAttr) {
        context.obtainStyledAttributes(attrs, R.styleable.RsImageView, defStyleAttr, 0)
                .apply {
                    type = getInt(R.styleable.RsImageView_rs_ib_type, TYPE_MASK)
                    style = getInt(R.styleable.RsImageView_rs_ib_style, STYLE_NORMAL)
                    normalResId = getResourceId(R.styleable.RsImageView_rs_normalResId, 0)
                    pressedResId = getResourceId(R.styleable.RsImageView_rs_pressedResId, 0)
                    disabledResId = getResourceId(R.styleable.RsImageView_rs_disabledResId, 0)
                    checkedResId = getResourceId(R.styleable.RsImageView_rs_checkedResId, 0)
                    uncheckedResId = getResourceId(R.styleable.RsImageView_rs_uncheckedResId, 0)
                    isChecked = getBoolean(R.styleable.RsImageView_rs_isChecked, false)
                    pressedAlpha = getFloat(R.styleable.RsImageView_rs_pressedAlpha,
                            DEFAULT_ALPHA_PRESSED)
                    disabledAlpha = getFloat(R.styleable.RsImageView_rs_disabledAlpha,
                            DEFAULT_ALPHA_DISABLED)
                    maskBackgroundColor = getColor(R.styleable.RsImageView_rs_maskBackgroundColor,
                            DEFAULT_MASK_BACKGROUND_COLOR)
                    cancelOffset = getDimensionPixelSize(R.styleable.RsImageView_rs_cancelOffset,
                            DEFAULT_CANCEL_OFFSET)
                    cornersRadius = getDimension(R.styleable.RsImageView_rs_cornersRadius, 0f)
                    cornersTopLeftRadius = getDimension(
                            R.styleable.RsImageView_rs_cornersTopLeftRadius, 0f)
                    cornersTopRightRadius = getDimension(
                            R.styleable.RsImageView_rs_cornersTopRightRadius, 0f)
                    cornersBottomLeftRadius = getDimension(
                            R.styleable.RsImageView_rs_cornersBottomLeftRadius, 0f)
                    cornersBottomRightRadius = getDimension(
                            R.styleable.RsImageView_rs_cornersBottomRightRadius, 0f)
                    recycle()
                }
        init()
    }
    ...
    private fun initType() {
        when (type) {
            ...
            TYPE_CHECKBOX -> {
                ...
                sld.addState(intArrayOf(android.R.attr.state_selected),
